...
class Model_lapharian extends Ci_Model
{
    ...
    public function __construct()
    {
        ...
        $this->prefs = array(
            ...
            'day_type'     => 'long',
            'show_next_prev' => TRUE,
            'next_prev_url'   => base_url() . 'manager/Lapharian/index/'
        );
        $this->prefs['template'] = '
		{table_open}
			<table border="0" cellpadding="0" cellspacing="0" class="table calendar table-bordered table-sm">{/table_open}

        {heading_row_start}<tr>{/heading_row_start}

        {heading_previous_cell}<th><a href="{previous_url}"><span class="fc-icon fc-icon-chevron-left"></span></a></th>{/heading_previous_cell}
        {heading_title_cell}<th colspan="{colspan}">{heading}</th>{/heading_title_cell}
        {heading_next_cell}<th><a class="fc-next-button fc-button fc-button-primary" href="{next_url}"><span class="fc-icon fc-icon-chevron-right"></span></a></th>{/heading_next_cell}

        {heading_row_end}</tr>{/heading_row_end}

        {week_row_start}<thead class="table-dark"><tr>{/week_row_start}
        ...
        {cal_cell_content}
        <div class="day_num fc-daygrid-day fc-day fc-day-tue fc-day-past">{day}</div>
        <div class="contentz">{content}</div>
        {/cal_cell_content}
        {cal_cell_content_today}
        <div class="">
        <div class="day_num fw-bolder">{day}</div>
        <span class="contentz badge rounded-pill badge-light-success">{content}</span>
    </div>
    {/cal_cell_content_today}
    ...
    {
        $query =  $this->db->select('produk.nama_produk,sum(penjualan.jumlah_stok) as total,produk.gambar_produk,detail_penjualan.tgl_trf')
            ->from('penjualan')
